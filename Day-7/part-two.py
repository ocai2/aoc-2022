...
def main():
    ...
    q = [root]
    smallest_but_big_enough_directory_size = 70000000
    space_needed = 30000000 - (70000000 - root.size)
    while len(q) > 0:
        d = q.pop()
        if d.size >= space_needed and d.size < smallest_but_big_enough_directory_size:
            smallest_but_big_enough_directory_size = d.size
        for c in d.children:
            q.append(c)
    
    print(smallest_but_big_enough_directory_size)
