...
def main():
    ...
    for i, x in enumerate(data):
        ...
        super_modulo = super_modulo * divisor
    ...
    for i in range(10000 * n_monkeys):
        for item in monkey_list[current_monkey].items:
            item = get_operated_item(item, monkey_list[current_monkey].operator, monkey_list[current_monkey].value, super_modulo)
...
def get_operated_item(item, operator, value, super_modulo):
    if value == "old":
        value = item
    else:
        value = int(value)
    ...
    if operator == "+":
        return (item + value) % super_modulo
    if operator == "-":
        return (item - value) % super_modulo
    if operator == "*":
        return (item * value) % super_modulo
    if operator == "/":
        return (math.floor(item / value)) % super_modulo
